...
export function DestinationList({
  ...
  const [displayiedDestinations, setDisplayiedDestinations] =
  ...
  useEffect(() => {
    ...
    setDisplayiedDestinations(tagFilter);
  ...
  return (
    <View className={`flex-wrap mx-4 flex-row justify-between pb-10`}>
      {loading && <Loading />}
      {!loading && displayiedDestinations.length === 0 && (
        <View className={`flex items-center w-full`}>
          <Text className={`text-lg`}>Sem resultado</Text>
        </View>
      )}
      {!loading &&
        displayiedDestinations.map((destination, index) => (
          <DestinationCard
            item={destination}
            key={destination.id}
