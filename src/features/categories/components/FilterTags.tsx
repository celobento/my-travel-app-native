...
import { TAG_DATA } from "@/src/values";
...
export function FilterTags({ activeTag, setActiveTag }: FilterTagsProps) {
  return (
    ...
    >
      <View
        className={`flex-row justify-around items-center mx-4 bg-neutral-200 rounded-full p-2 px-4 space-x-2`}
      >
        {TAG_DATA.map((tag, index) => {
          let isActive = tag.id === activeTag;
