...
import { IMAGE_SOURCES } from "@/src/values";
...
export type CategoriesProps = {
  activateCategory: number | null;
  setActivateCategory: (value: number | null) => void;
};
    ...
    <View className={`space-y-5`}>
      ...
      >
        {categories.map((category, index) => {
          ...
          const isActive = category.imageId === activateCategory;
          return (
            <TouchableOpacity
              key={category.imageId}
              className={`flex items-center stapce-y-2`}
              onPress={() => setActivateCategory(category.imageId)}
            >
              <View
